...
function drawAestheticRatios() {
	const domAesthetics = document.querySelector('#aestheticRatios g');
	const aestheticRatios = [
		{ label: 'VC', ratio: 1.128 },
		{ label: 'PH', ratio: 1.414 },
		{ label: 'GR', ratio: 1.618 },
		{ label: 'SR', ratio: 2.414 },
	];

	function createAestheticSvg(deg, idx) {
		const MINOR_CIRCLE = 3;
		const MAJOR_CIRCLE = 20;
		const TEXT_OFFSET = 6;
		const LABEL_OFFSET = 8;
		...
		const argAngle = Math.atan(deg.ratio);
		const arcRadius = (idx + 1) * ARC_INTERVAL + ARC_OFFSET;
		const arcDir = (deg.ratio < 1.6) * 1;
		const lineX = arcDir ? CANVAS_HEIGHT * deg.ratio : CANVAS_WIDTH;
		const lineY = arcDir ? 0 : CANVAS_HEIGHT - CANVAS_WIDTH / deg.ratio;
		const arcStartX = arcDir ? 0 : arcRadius;
		const arcStartY = CANVAS_HEIGHT - (arcDir ? arcRadius : 0);

		const arcEndX = arcRadius * Math.sin(argAngle);
		const arcEndY = CANVAS_HEIGHT - arcRadius * Math.cos(argAngle);

		return `
		<line x1="0" y1="${CANVAS_HEIGHT}" x2="${lineX}" y2="${lineY}"/>
		<circle cx="${lineX}" cy="${lineY}" r="${MAJOR_CIRCLE}"></circle>
		<text x="${lineX}" y="${lineY + LABEL_OFFSET}">${deg.label}</text>
		<path d="M ${arcStartX} ${arcStartY} A ${arcRadius} ${arcRadius} 0 0 ${arcDir} ${arcEndX} ${arcEndY}"/>
		<circle cx="${arcEndX}" cy="${arcEndY}" r="${MINOR_CIRCLE}"></circle>
		<circle cx="${arcStartX}" cy="${arcStartY}" r="${MAJOR_CIRCLE}"></circle>
		<text x="${arcStartX}" y="${arcStartY + TEXT_OFFSET}" class="value">${
			deg.ratio
		}</text>
		`;
	}

	domAesthetics.innerHTML = aestheticRatios.map(createAestheticSvg).join('');
}
